#!/usr/bin/env node
/**
 * Общая часть всех упаковок: `dist/main.js` + `dist/diode.bundle`.
 *
...
import { buildDiodeBundle } from "./pack-assets.mjs";
...
export async function buildDistArtifacts({ repoRoot }) {
    ...
    // 3. Пакуем onig.wasm + src/Extensions/builtin/** в единый dist/diode.bundle.
    const { bundle, inputs } = buildDiodeBundle({ repoRoot });
    const bundlePath = join(distDir, "diode.bundle");
