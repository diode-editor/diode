...
const HOVER_FG = packRgb(240, 240, 240);
...
describe("ListViewElement styling", () => {
    ...
    it("hover uses hoverBg/hoverFg on a non-cursor row", () => {
        ...
        expect(app.backend.getFgAt(new Point(0, 1))).toBe(HOVER_FG);
    });
    ...
    it("hover keeps the normal fg when hoverFg is undefined", () => {
        const list = new ListViewElement();
        list.setStyles({ ...unthemedListViewStyles, hoverBg: HOVER_BG, hoverFg: undefined });
        list.appendRow(makeRow("a", "Alpha"));
        list.appendRow(makeRow("b", "Beta"));
    ...
    it("hover is disabled when hoverBg is undefined", () => {
        const list = new ListViewElement();
        list.appendRow(makeRow("a", "Alpha"));
        list.appendRow(makeRow("b", "Beta"));
        const app = TestApp.createWithContent(list, new Size(20, 5));
