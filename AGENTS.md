...
- docs/PR.md — что обязано быть в PR: демо-сценарий и ссылка на сборку
- office/README.md — офис агентов: конвейер от заявки до влитого PR (манифест — office.yaml)
- docs/iterations/ — журналы итераций и ретроспективы (индекс — README.md)
- docs/TODO/ — трекер задач (индекс — README.md)
- docs/public/ — публичные страницы, на которые ссылается сайт (бенчмарки, матрица готовности API); правила каталога — в его README.md
...
# Офис агентов
Конвейер, которым работа идёт от заявки человека до влитого PR, объявлен в `office.yaml` и описан в [office/README.md](office/README.md): диспетчер разводит заявку по курсам (баг / фича / цель), дальше реализация → сверка с эталоном vscode → тестирование → ветка итерации, а в конце итерации — один большой PR со ссылкой на сборку. Офис поднимается `npm run office`, петля проверяется на заглушках `npm run office:dry`.

Если ты работаешь ролью офиса, твой промпт лежит в `office/roles/<роль>.md`, и правила этого файла для тебя не отменяются — офис поверх них, а не вместо. Итерации, ветки заявок (`task/<узел>`) и журнал итерации ведёт `scripts/iteration.mjs`: руками ветку итерации не собирай.
...
# Style
Линт и типы — гейт CI (job `static` в `.github/workflows/ci.yml`): красный `npm run lint` или `npm run typecheck` блокирует PR, поэтому перед сдачей прогоняй оба. Форматирование и сортировку импортов чинит `npm run lint:fix`, остальное — руками.

Правило `no-unnecessary-condition` часто ругается на защитные проверки чтения по индексу: `noUncheckedIndexedAccess` в `tsconfig.json` выключен (включение даёт ~2700 ошибок типов — отдельная задача), поэтому `arr[i]` для TS всегда непустой. Не выбрасывай проверку — сделай её честной: `.at(i)` там, где индекс неотрицательный (у `.at(-1)` семантика другая — отсчёт с конца!), `Partial<Record<…>>` для справочников с произвольным ключом, явная проверка границ там, где индекс бывает отрицательным.

Не забывай использовать расширения в имени файла при импорте.
...
- Что обязано быть в PR — [docs/PR.md](docs/PR.md). Любая фича с видимой/внешней составляющей обязана принести **сценарий-демо** в `e2e/scenarios/`. Это не формальность: в #194 пропущенный сценарий стоил бага, который виден на первом же кадре. Сценарию, которому нужно стоковое расширение, отдай `installVsix` (см. `e2e/scenarios/regionFolding.scenario.ts`). Скриншот в теле PR обязательным больше не является: работу принимают на собранном бинарнике из PR (`PR Build`), а не по картинке.
...
- **Перед сдачей фичи — `npm run test:mutation`.** Вторая половина храповика: покрытие говорит «строка исполнилась», мутационный балл — «кто-то её проверил». Тест без осмысленного ассерта даёт 100% покрытия и 11% мутационного балла (замерено). Выжившего мутанта либо убиваем тестом, либо гасим `// Stryker disable next-line <мутатор>: причина` — но не ассертом ради балла. Детали → [docs/TESTING.md](docs/TESTING.md#мутационное-тестирование-stryker).
...
- **Фича поверх стокового расширения закрывается стоковым расширением.** Поднять настоящий `.vsix` мало — надо дёрнуть его настоящую функциональность (команду, провайдер) из пользовательского состояния (курсор подвинут, текст выделен) и проверить результат. Фикстура, выведенная из своей же реализации, проверяет ровно то, что и так работает. Само расширение при этом приезжает **из магазина** (последняя опубликованная версия; e2e/сценарии — установка по id, юнит-сьюты — fetch-хелпер с кэшем), а не закоммиченным `.vsix`: версию в тестах не пиним, сломалось после обновления записи реестра — бежим чинить. Герметично — только редактор и API расширений, на своих синтетических расширениях; сетевой стоковый сьют при этом не может быть единственным гейтом нашего контракта — такой контракт закрывается отдельным герметичным тестом. Конвенция → [docs/TESTING.md](docs/TESTING.md#тесты-на-стоковые-расширения--из-магазина-конвенция).
...
- **Ограничения tuidom НЕ обходим — идём и чиним движок.** Движок приходит пакетами `@tuidom/*` из npm; если фиче не хватает возможности движка или виджета (нет API/хука, поведение не параметризуется, виджет не расширяется штатным наследованием с нашей стороны) — недостающее делается **в репозитории tuidom** (`/workspaces/tuidom/tuidom`, github.com/tuidom/tuidom): своя ветка, свой PR, вливаешь его сам по правилам того репозитория. Останавливаться и ждать человека для этого не нужно — но **публикацию новой версии в npm запрашивай у него** (вопросом с вариантами) и до выхода версии не завязывай на неё diode. Запрещённые обходы остаются запрещёнными: копировать куски движка в diode, monkey-patching и лазанье в приватные поля (`as any`, `@ts-expect-error`), дубликат виджета у нас ради недостающего хука, пост-обработка отрендеренного grid'а в обход каскада. Тест-обходы — тоже обходы.
