...
При открытии большой директории Diode тормозил: медленное открытие воркспейса и
...
- [ ] **Нативный обход для `ITreeFileWatcher` (`@parcel/watcher` вместо chokidar).**
      Второй из двух ортогональных рычагов VS Code; первый — «обход в своём процессе» —
      уже сделан (`SubprocessTreeWatcher`, см. [ARCHITECTURE.md](../ARCHITECTURE.md#роли-процессов-форк-самого-себя)),
      и именно он снимал просадку отклика. Нативный обход отклика уже не улучшит —
      он про **суммарную** цену: сейчас обход стоит ~14 с CPU на дереве с ~55k
      каталогов (`readdirp` + `stat` на каждый вход + JS-глоб на каждый путь), в
      C++-реализации это кратно дешевле, а батарея и время до «слежение живо» —
      единственное, что от этого выигрывает. Лимит inotify нативный бэкенд **не**
      поднимает: у `@parcel/watcher` на Linux под капотом тот же inotify, и VS Code
      ловит там тот же ENOSPC.
      Цена входа — нативный модуль под SEA-упаковку: рецепт есть (`node-pty` →
      `dist/node-pty.bundle` + `loadNodePty.ts`: распаковка `.node` в кэш + `dlopen`,
      сборка — `scripts/build-sea.mjs`), но `@parcel/watcher` приезжает отдельным
      пакетом на каждую платформу и libc (`@parcel/watcher-linux-x64-glibc` /
      `-musl` и т.д.), так что это ещё и платформенная матрица сборки.
      Брать, когда суммарный CPU станет ощутимой проблемой, — но не раньше, чем
      будет закрыто уведомление про ENOSPC (см. [EnvironmentTuning](EnvironmentTuning.md)):
      молчаливый отказ слежения бьёт пользователя сильнее, чем его цена.
