...
- [x] **PR-1. Вкладка-снимок + «Git: Open File at Revision...»** — сделано.
  ...
  команды — освежает).
- [ ] **PR-2. View zones** — единственный инфраструктурный PR. Примитив в
  `EditorViewState` (вставные виртуальные строки в индирекции view↔doc рядом с
  фолдингом) + рендер/hit-test/каретка в `EditorElement`. Гейт: юниты маппингов
  и рендера; немедленно валидируется PR-3.
- [ ] **PR-3. DiffEditorPane v2 — side-by-side из двух настоящих редакторов**
  (пока снимок): зоны-филлеры, внешние декорации (п.2 инвентаря втягивается
  сюда, не отдельным слепым PR), скролл-синк, маркеры в гуттерах, свёртка
  unchanged фолдингом, intra-line. Первая большая видимая точка — прогоняет
  view zones в настоящем приложении.
