...
| `languages.createDiagnosticCollection` | naive | работает: notify `diagnostics.publish` → `diagnosticsSink` → `MarkerService.changeOne` (squiggle + Problems); rich-форма `code: { value, target }` (так шлёт eslint — код правила + ссылка на доку) уезжает своим value, target TUI некуда открывать; наивность — related information не передаётся, маркеры мёртвого subprocess'а не сбрасываются до рестарта. **Открытый дефект (найден на демо `diagnostics-guides`)**: класса-ловушки `DiagnosticRelatedInformation` в стабе `vscodeTypes.ts` НЕТ, хотя в `vscode.d.ts` он раскомментирован. Конвертер vscode-languageclient конструирует его на каждую диагностику с related information (TS2741 «Property … is missing», дубликаты идентификаторов — сплошь и рядом), падает `TypeError: code.DiagnosticRelatedInformation is not a constructor`, и **вся пачка диагностик теряется** — файл остаётся без squiggle вовсе, ошибка видна только в output-канале клиента (`Processing diagnostic queue failed`). Лечится как прочие ловушки: класс в стаб (+ `Location` уже есть) и тест по образцу `vscodeTypes.lsp.test.ts` |
