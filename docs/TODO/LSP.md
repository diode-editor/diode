...
| `window.createStatusBarItem` | real | пункт в полосе поверх `StatusBarService` (`ExtensionStatusBarAdapter`, notify `window.statusBarItem.update`/`.dispose`): `text` со значками `$(name)`, `name` (он же строка меню видимости), `alignment`, `priority`, команда по клику с аргументами, `show`/`hide`/`dispose`. Состояние живёт в субпроцессе, `update` несёт полное состояние; непоказанный пункт в проводе не появляется. Id записи — `extensions.<id>`: встроенные сегменты расширению недоступны. На смерть subprocess'а host сам снимает пункты, как и спиннеры. Стабы: `tooltip` (виджета подсказки в TUI нет), `color`/`backgroundColor` (нет токенов темы и посегментных стилей), `accessibilityInformation`; текст режется на 24 символах — ширина полосы дефицитна |
