...
- **Синхронизируется только активный редактор** — работа со сплитами/фоновыми
  документами не передаёт их правки. Сплиты уже есть ([EditorGroups](EditorGroups.md));
  фикс — подписка didOpen/didChange пер-МОДЕЛЬ + `editor.didClose` — идёт фазой
  API-A/AS-11 той задачи.
