...
- [x] [SearchPerformance](SearchPerformance.md) — тормоза курсора в дереве результатов поиска устранены: кап `preview.after` у истока, кэш `DisplayLine` в лейбле, dirty-гейт кадра ввода (1 кадр на нажатие), виртуализация стилевого прохода списка, damage-tracking кадра (случай 4); 538 мс → 0.9 мс на итерацию бенча. Открыто: случай 6 (пересборка проекции при стриме — терпимо)
