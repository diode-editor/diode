...
- [~] [Marketplace](Marketplace.md) — курируемый магазин расширений: формат реестра (index + meta, sha256, engines), файловый источник + `--registry`/install-by-id **готовы**; далее registry-репо с наполнением через PR, система тестирования расширений, HTTP-источник, Extensions view (заменяет старую модель Phase 10 из [Extensions](Extensions.md))
