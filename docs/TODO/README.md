...
- [ ] [ExtensionsView](ExtensionsView.md) — просмотр магазина из редактора: вьюлет `EXTENSIONS` в сайдбаре (поиск по каталогу + секция установленных), страница расширения отдельной вкладкой (метаданные + readme текстом), установка/обновление/удаление кнопкой с сообщением о перезапуске. Собраны пользовательские сценарии US-1…US-21 (приёмочный чек-лист) и эскиз устройства; реализации ещё нет
