...
- [~] [EditorGroups](EditorGroups.md) — сплиты области редактора: сетка групп как в VS Code, разбиение только командами/хоткеями, `ViewColumn`/`showTextDocument`/`window.tabGroups` в API расширений. Готова постановка — пользовательские сценарии US-1…50 и API-сценарии AS-1…20 плюс номенклатура команд; реализация не планировалась
