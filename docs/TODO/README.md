...
### [ ] Матрица покрытия API VS Code — генерить из vscode.d.ts
Публичная матрица «namespace/поверхность → поддержано / частично / запланировано / не будет
by design». Источник правды уже есть: активная поверхность — **дословно раскомментированные
строки** запиннённого `vscode.d.ts` (см. AGENTS.md, docs/arch/Extensions.md), значит матрица
собирается скриптом и не может врать или устаревать. Графа «не будет by design» (webview,
UI-heavy) обязательна — честный потолок вслух. Заготовка страницы —
[docs/public/API-COVERAGE.md](../public/API-COVERAGE.md); осталось написать генератор и заменить
примерные строки. Ссылки с главной и из роадмапа.
