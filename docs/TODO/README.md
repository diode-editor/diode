...
- [~] [LSP](LSP.md) — стоковые language servers поверх extension host (клиент — builtin-расширение на `vscode-languageclient`); document sync (didOpen/didChange) готов; далее definition-провайдер + F12, стабы под languageclient, диагностики → MarkerService, закрытие стоковым `typescript-language-server`
...
- View GRAPH: настоящий граф коммитов (рёбра, ветки) вместо плоского списка,
  активация коммита (показ диффа/деталей).
