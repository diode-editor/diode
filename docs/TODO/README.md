...
- [~] [EditorGroups](EditorGroups.md) — сплиты области редактора: полоса групп по одной оси (Ctrl+\, фокус Ctrl+1..5/чорды, перенос/копия/join, resize/maximize, ось-тумблер, Open to the Side), undo и модель — на документ (реестр с ref-count), find-виджет на группу, персист полосы. Ядро (фазы 1–7) готово; осталось API расширений: ViewColumn/showTextDocument/window.tabGroups (фазы API-A/API-B по фазировке в документе)
