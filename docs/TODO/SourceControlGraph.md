...
- **Палитра** (`commitGraphPalette.ts`) — модель VS Code, не lazygit: коммит, несущий
  текущую ветку или remote-ветку, задаёт своей линии семантический цвет, остальные
  разбирают пять токенов `scmGraph.foreground1..5` по кругу. Авторский `md5 → HSL`
  lazygit несовместим с системой тем и не взят.
- **Данные** — прежний push-канал `vexx.scm.publishLog`, payload расширен до
...
- **Строка** — `[граф][бейджи refs][subject]`. **Колонки sha нет**: в сайдбаре шириной
  ...
  Ширина графовой колонки общая для всех строк (иначе бейджи разъедутся по вертикали) и
  зажата потолком `GRAPH_MAX_WIDTH` — семь дорожек.
