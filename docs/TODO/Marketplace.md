...
- [x] Минимальный semver (`src/vs/base/common/semver.ts`): `*`, точная, `^`, `~`,
  `>= > <= <`, конъюнкция; прочее осознанно `false`.
...
- **CI-валидация PR:** схема (нашими же парсерами из `registryFormat.ts`), id == имя
  файла, артефакт скачивается, пересчёт и сверка `sha256`/`size`, `engines` парсится
  нашим semver-подмножеством, иммутабельность (изменение опубликованной версии —
  отказ).
