...
Тот же принцип — у всех сьютов на **стоковые** расширения (`extensionHost.pythonLsp*`,
`e2e/pythonLsp.test.ts`, сценарий `python-lsp`): расширение приезжает из магазина — последняя
совместимая версия, установка по id через `--install-extension` (e2e/сценарии) либо
`fetchBasedpyrightVsix` с кэшем в `node_modules/.cache` (юнит-сьюты ext-host'а). Закоммиченных
сторонних `.vsix` не держим и версию в репозитории не пиним: функциональность редактора и API
расширений тестируем герметично на СВОИХ синтетических расширениях, а стоковые — тем, что реально
опубликовано; герметичность здесь жертвуется сознательно. Обновилась запись в реестре и сьюты
покраснели — это не шум, а сигнал бежать и чинить (или совместимость diode, или запись магазина).
`DIODE_E2E_OFFLINE=1` пропускает и эти сьюты — включая сценарии с пометкой `network: true`.
