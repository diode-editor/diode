...
> **Гейт умеет флейкать.** Тот же коммит от прогона к прогону даёт разные наборы выживших: подбор через `vitest.related` находит не все покрывающие тесты. Прежде чем гасить выжившего, прогони его локально тем же флагом, что CI (`npx stryker run --mutate '<файл>' --ignoreStatic`) — если локально он убит, это флейк, а не дыра в тестах. Разбор и улики — [TODO/MutationGateFlake.md](TODO/MutationGateFlake.md).
...
код валит от одного до четырёх тестов.

Теряется не случайный прогон, а **следующий за оборванным по bail**. Stryker
гоняет vitest с `bail: 1`, поэтому прогон убитого мутанта обрывается на первом
же упавшем тесте; следующий прогон в том же переиспользуемом воркере стартует и
тихо не доезжает: файлы остаются в состоянии `run` без результатов, а раннер
отбрасывает всё без результата (`.filter((test) => test.result)`) и получает
пустой список. Инструментированный прогон по одному файлу: оба потерянных
прогона шли ровно следом за прогоном с `fail: 1`. Апстрим знает —
...
`disableBail` симптом убирает, но ценой прогона всех покрывающих тестов на
каждого мутанта: на `progressService.ts` это ~1 мутант в минуту вместо ~1 в
секунду, то есть гейт из пятнадцати минут превращается в пару часов.
...
отчёт: упавшие перепроверяются точечно наравне с потерянными, а если раннер
падает и там — гейт краснеет и печатает мутанта.
...
с локализацией ошибок слушателя (как `onUnexpectedError` у vscode): руками
раскатанных «эмиттеров» у нас около тридцати файлов, и ни один ошибку слушателя
не ловит — это #275.

Пока общего эмиттера нет, у конкретного мутанта остаётся штатный выход —
`// Stryker disable next-line <мутатор>: причина` с честной причиной («раннер
падает на unhandled error из микротаска, см. #275»). Это ровно тот случай, для
которого гашение и придумано: мутант указывает на дыру в архитектуре событий, а
не в тестах этой строки.
