...
- `checks.ts` — необязательные поведенческие смоуки: чек дёргает настоящую функциональность и
  смотрит на состояние редактора через инспектор (`EditorElement.state.tabSize`), без ввода в PTY,
  поэтому кроссплатформенно. Глубина видна в имени кейса: «ставится из магазина» против «ставится из
  магазина и работает в редакторе».
