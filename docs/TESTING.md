...
`e2e/marketplace/` — система тестирования расширений реестра. Проходит полный цикл против
**настоящего** опубликованного реестра (`https://diode-editor.github.io/registry/v1/`, адрес берётся
из клиентского `DEFAULT_REGISTRY_URL`, а не из своей копии строки): индекс и мета по сети → артефакт
(наш с Pages либо чужой с open-vsx) → `sha256` → установка собранным бинарём → запуск редактора →
проверка функциональности расширения.

- `checks.ts` — по смоук-чеку на запись реестра. Чек дёргает настоящую функциональность и смотрит на
  состояние редактора через инспектор (`EditorElement.state.tabSize`), без ввода в PTY — поэтому
  кроссплатформенно. **Гейт курирования:** тест сверяет список чеков со списком опубликованных
  расширений, так что запись, добавленная в реестр без проверки в Diode, красит сьют.
- Версия из `Installed <id>@<version>` сверяется с `latest` в индексе: обновление записи в реестре
  тоже требует зелёного прогона.
