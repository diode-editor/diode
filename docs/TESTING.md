...
Фича с видимой/внешней составляющей обязана добавить/обновить сценарий в `e2e/scenarios/` и посмотреть на его кадры своими глазами. PNG к телу PR прикладывать не обязательно: работу принимают на собранном бинарнике из `PR Build` (правила — [AGENTS.md](../AGENTS.md), [PR.md](PR.md)).
...
взят — он самый частый, и его сериализация стоит дороже; если полный юнит-прогон начнёт давать
плавающие таймауты в subprocess-тестах, смотреть надо сюда же.
...
У точечного прогона есть своя ловушка, и она стоила красного гейта в #339.
Stryker пишет `reports/mutation/mutation.json` в самом конце, а начинает с
**initial test run** — а там гоняется весь сьют, включая сетевые тесты стоковых
расширений. Флак там (у нас это был 60-секундный таймаут `references` от
typescript-language-server) роняет точечный прогон целиком, до отчёта он не
доходит — и на диске остаётся отчёт ПЕРВОГО прогона. Скрипт читал его как
результат перепроверки и объявлял «эти мутанты упали и в точечном прогоне» там,
где точечного прогона не было вовсе. Теперь файл отчёта убирается с дороги перед
вторым прогоном: нет свежего отчёта — значит вердикта нет, прогон повторяется
один раз, а если и повтор не доехал, гейт краснеет с этой формулировкой, а не с
обвинением в адрес кода.
