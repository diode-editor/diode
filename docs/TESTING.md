...
Движок вынесен в отдельный репозиторий ([github.com/tuidom/tuidom](https://github.com/tuidom/tuidom)) и приходит пакетами `@tuidom/*` — его собственные тесты живут там. В vexx остаются тесты **потребления** движка: компоненты поверх виджетов, интеграция с приложением.

Тест-харнесс приходит из пакета (`@tuidom/testing/*`); vexx-обёртки `src/TestUtils/{TestApp,renderElement}.ts` дополняют его **живой палитрой Dark+** (`computeThemeVars` — тот же код темы, что в приложении; сам пакет по умолчанию использует data-снапшот `testing/darkPlusStyleVars`, регенерируемый из vexx). Пользуйся обёртками, не пакетным харнессом напрямую — иначе тест увидит снапшот вместо живой темы.
