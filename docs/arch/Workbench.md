...
- Четвёртая ось — **ОС клавиатуры** и её **мак-лестница**. `os` — ОС клавиатуры,
  ...
  терминала → локальный `darwin`). Рядом с tier строится отдельная мак-лестница
  рунгов `legacy < extended < cmd` (числовой ключ `macKeys`, 0 — не мак). Её
  решающая способность — доезжает ли Cmd (`cap_super`), и ломает её не tier, а
  tmux: под ним рунг не выше `extended`. В `tier` мак не засовывается, потому
  что «мак» не мощнее «pc». Авторы биндов пишут типизированные хелперы
...
- **`setContext`** — встроенная команда VS Code, которой РАСШИРЕНИЯ публикуют свои
  ...
  нормализуется до `boolean | string | number` (`null`/`undefined` → `false`;
  массив/объект → своя истинность — оператора `in` у нашего вычислителя нет), имя
  ключа перед записью регистрируется через `registerContextKeys`.
- **Точечные имена ключей в when-вычислителе.** `ContextKeyService.evaluate`
  компилирует выражение в `new Function("__scope", "with (__scope) { return !!(…) }")`.
  `with`, а не список параметров: имя ключа приходит от расширения
  (`setContext`) и параметром быть не обязано (`foo-bar`, `2fa`, `class`) —
  такое имя развалило бы КОМПИЛЯЦИЮ, то есть все when-выражения сразу, а не
  только своё. В теле `new Function` `with` законен: функция всегда
  компилируется в sloppy-режиме, независимо от строгости модуля-создателя.
  Точечное имя (`supermaven.isProUser`) `buildScope` раскладывает ВЛОЖЕННЫМИ
  объектами под корневым сегментом — в выражении это обычное чтение свойства, и
  текст when-клаузы не меняется. Скоуп — `Object.create(null)`: ключ с именем
  `__proto__` иначе уехал бы в сеттер прототипа вместо значения. Столкновение
  корня с плоским ключом разрешается в пользу плоского (в примитив вложить
  нечего). Настоящий парсер when-выражений вместо `new Function` — в
  [../TODO/WhenContext.md](../TODO/WhenContext.md).
