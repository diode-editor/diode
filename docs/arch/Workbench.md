...
- **Точечные имена ключей в when-вычислителе.** `ContextKeyService.evaluate`
  компилирует выражение через `new Function`, где имена ключей становятся
  ПАРАМЕТРАМИ. Имена расширений точечные (`supermaven.isProUser`), параметром
  такое имя быть не может — поэтому `buildScope` отдаёт вместо него ВЛОЖЕННЫЙ
  объект под корневым сегментом: в выражении это ровно обычное чтение свойства,
  и текст when-клаузы не меняется. Имя, негодное в параметр (`foo-bar`, `2fa`,
  `class`), молча не попадает в скоуп — значение хранится, но вычислителю не
  видно; уронить компиляцию нельзя, это убило бы ВСЕ when-выражения сразу.
  Столкновение корня с плоским ключом разрешается в пользу плоского. Настоящий
  парсер when-выражений вместо `new Function` — в [../TODO/WhenContext.md](../TODO/WhenContext.md).
