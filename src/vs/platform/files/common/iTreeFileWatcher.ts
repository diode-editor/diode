...
export interface ITreeFileWatcher {
     ...
     *
     * Каждый вызов — независимая подписка со своими excludes и своей
     * рекурсивностью; делит ли реализация под ними один обход дерева, снаружи
     * не видно (`SharedTreeWatcher` делит — обход дорог, и платить за него
     * дважды незачем). Соответственно `dispose()` гасит только свою подписку.
     */
    watchTree(
        rootPath: string,
