...
describe("parseRegistryIndex", () => {
    ...
    it("generatedAt не-строка отбрасывается, contract не ломается", () => {
        const { index } = parseRegistryIndex(indexText([], { generatedAt: 42 }));
        expect(index.generatedAt).toBeUndefined();
    });

    it.each([
        ["не JSON", "{oops", /malformed JSON/],
        ["не объект", "[1,2]", /expected a JSON object/],
        ["нет schemaVersion", "{}", /missing "schemaVersion"/],
        ["schemaVersion строкой", '{"schemaVersion":"1"}', /missing "schemaVersion"/],
        [
            "schemaVersion новее клиента",
            JSON.stringify({ schemaVersion: REGISTRY_SCHEMA_VERSION + 1, extensions: [] }),
            /update Diode/,
        ],
        ["нет extensions", JSON.stringify({ schemaVersion: 1 }), /missing "extensions" array/],
        ["extensions не массив", JSON.stringify({ schemaVersion: 1, extensions: {} }), /missing "extensions" array/],
...
describe("parseRegistryMeta", () => {
    ...
    it.each([
        ["битая идентичность", metaText([], { kind: "bad" }), /bad identity fields/],
        ["versions не массив", metaText([], { versions: "none" }), /missing "versions" array/],
    ])("битый файл (%s) — ошибка", (_label, text, error) => {
...
describe("searchRegistryIndex", () => {
    ...
    it.each([
        ...
        expect(searchRegistryIndex(index, query).map((e) => e.id)).toEqual(expected);
    });
});
