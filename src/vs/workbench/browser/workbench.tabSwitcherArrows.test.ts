...
describe("Workbench — стрелки Вверх/Вниз в видимом списке Ctrl+Tab", () => {
    ...
    it("одна открытая вкладка: списка нет, Ctrl+Вниз прокручивает редактор", () => {
        ...
        try {
            ...
            // Ctrl+Tab при одной вкладке не резолвится (`editorTabsMultiple`) и
            // достаётся дефолтному обходу фокуса по Tab — как и на main до этой
            // задачи. Возвращаем фокус в текст и проверяем то, что относится к
            // стрелкам: без видимого списка Ctrl+Вниз — это прокрутка.
            one.workbench.focusEditor();
