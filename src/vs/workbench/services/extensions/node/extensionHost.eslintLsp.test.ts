...
describe.skipIf(MARKETPLACE_OFFLINE)("ExtensionHost — стоковый vscode-eslint.vsix (JS-линт, сквозняк)", () => {
    ...
    it("активация без ошибок, no-extra-semi приезжает и уходит после несохранённой правки", { timeout: 240_000 }, async () => {
        ...
        try {
            ...
            const extraSemi = await until("диагностика no-extra-semi в lint.js", () => {
                const hit = markersFor(lintUri).find((m) => String(m.code ?? "") === "no-extra-semi");
                return Promise.resolve(hit ?? null);
            });
            expect(extraSemi.startLine).toBe(0);
            expect(markersFor(lintUri).some((m) => String(m.code ?? "") === "no-unused-vars")).toBe(true);
            ...
            harness.group.getActiveEditor()?.applyExternalEdits(
                [createTextEdit(createRange(0, 17, 0, 18), "")],
                "drop extra semi",
            );
            await until("no-extra-semi ушла после правки", () =>
                Promise.resolve(
                    markersFor(lintUri).some((m) => String(m.code ?? "") === "no-extra-semi") ? null : true,
                ),
            );
