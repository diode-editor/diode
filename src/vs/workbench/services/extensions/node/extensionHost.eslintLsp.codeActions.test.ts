...
describe.skipIf(MARKETPLACE_OFFLINE)("ExtensionHost — code actions от стокового vscode-eslint", () => {
    ...
    it("editor.action.fixAll применяет source.fixAll.eslint: лишняя `;` ушла, no-unused-vars остался", { timeout: 240_000 }, async () => {
    ...
    it("quickfix по диагностике no-extra-semi: фикс с isPreferred, apply правит буфер", { timeout: 240_000 }, async () => {
        const { harness, lintUri } = await openLintFile();
        try {
            ...
            const quickfixes = await until("quickfix-набор для no-extra-semi", async () => {
                ...
                return found.some((a) => /no-extra-semi/.test(a.title)) ? found : null;
            });

            const fix = quickfixes.find((a) => /^Fix this no-extra-semi/.test(a.title));
